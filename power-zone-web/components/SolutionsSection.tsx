        ...
        <div className="absolute inset-x-0 bottom-0 top-[clamp(180px,28vh,280px)] z-10 flex items-center overflow-hidden">
          ...
          >
            {SOLUTION_CARDS.map((card, i) => (
              <article
                key={card.title}
                className="pz-sol-card relative flex-shrink-0 overflow-hidden rounded-3xl bg-white shadow-[0_30px_80px_-20px_rgba(0,0,0,0.55),0_10px_30px_-10px_rgba(0,0,0,0.4),0_0_60px_-15px_rgba(220,38,38,0.18)]"
                style={{
                  // Bumped ~15% from the previous clamp so the title +
                  // description have room to breathe at a more readable
